...
class ViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "editItem" {
            let navigationController = segue.destination as? UINavigationController
            let editItemViewController = navigationController?.topViewController as? AddGolfViewController

            if let viewController = editItemViewController {
                viewController.delegate = self as AddGolfViewControllerDelegate
            }
        }
    }
...
extension ViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            let game = GolfGame(id: indexPath.row, name: indexPath.description, numberOfHoles: indexPath.row)
