...
class AddNoteViewController: UIViewController {
    ...
    @IBAction func saveButton(_ sender: UIBarButtonItem) {
        ...
        if isPressentingInEditNoteMode {
            if let note = Note(id: userNotes.incrementIdentifierByOne(), name: textField.text ?? "") {
        ...
        if (!isPresentingInAddNoteMode) {
            if let owningNavigationController = navigationController {
                owningNavigationController.popViewController(animated: true)
                if let noteEdit = Note(id: userNotes.editNoteID(note!), name: textField.text ?? "") {
//                    delegate?.controllerDidEdit(self, didEdit: noteEdit)
//                    userNotes.editNote(noteEdit)
                    userNotes.changeNote(noteEdit)
                }
                print("You pressed the Save button in EditNote screen")
            }
        } else {
            print("The NoteViewController is not inside a navigation controller.")
        }
    }
}
