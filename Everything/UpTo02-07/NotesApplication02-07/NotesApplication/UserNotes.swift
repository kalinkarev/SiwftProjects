...
//    mutating func nameChangeAtIndex(element: Note?, index: Int) {
//        for i in notes {
//            if i.id == notes[index].id {
//                notes[index].name = (element?.name)!
//            }
//        }
//    }
    
//    mutating func changeTheNamePropertyOfObject(noteForEdit: Note, index: Int) {
//        if let index = notes.index(where: {$0.id == noteForEdit.id}) {
//            notes[index].name = noteForEdit.name
//        }
//    }
    

    mutating func arrayChangedPropertyName(element: Note) {
        
        for i in 0..<notes.count {
            if notes[i].id == element.id {
                notes[i].name = element.name
            }
        }
        print("You have changed the name of a element in the array")
    }
